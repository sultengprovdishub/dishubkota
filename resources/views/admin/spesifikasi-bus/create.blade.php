...
@section('title', 'Tambah Spesifikasi Bus')
@section('page-title', 'Tambah Spesifikasi')
@section('page-subtitle', 'Tambah item spesifikasi bus baru')
...
@section('content')
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2">
            <form action="{{ route('admin.spesifikasi-bus.store') }}" method="POST">
                @csrf
                ...
                <div class="bg-white rounded-2xl shadow-card overflow-hidden">
                    <div class="px-6 py-4 border-b border-gray-100 flex items-center gap-3"
                        ...
                        <div>
                            <div class="text-white font-bold text-sm">Data Spesifikasi</div>
                            <div class="text-purple-200 text-xs">Informasi teknis armada bus</div>
                        </div>
                    </div>
                ...
                <div class="flex gap-3 mt-4">
                    <button type="submit" class="btn-primary flex-1 justify-center py-3">
                        <i class="bx bx-save mr-1"></i> Simpan Spesifikasi
                    </button>
        ...
        <div class="space-y-4">
                    ...
                    <div id="spek-preview-card"
                        ...
                        <div class="flex items-start gap-3">
                            <div id="prev-icon-wrap"
                                style="width:42px;height:42px;border-radius:12px;background:#f5f3ff;flex-shrink:0;display:flex;align-items:center;justify-content:center;transition:all .3s;">
                                <i id="prev-icon" class="bx bx-cog"
                                    style="font-size:20px;color:#7c3aed;transition:all .3s;"></i>
                            </div>
                            <div>
                                <div id="prev-label" class="text-xs font-semibold text-gray-400 uppercase tracking-wide">
                                    Label</div>
                                <div id="prev-nilai" class="font-bold text-gray-900 text-base mt-0.5">Nilai</div>
                            </div>
                        </div>
                    </div>
            ...
            <div style="background:#faf5ff;border-radius:16px;padding:16px;border:1px solid #e9d5ff;">
                <div class="text-xs font-bold text-purple-800 mb-2 flex items-center gap-2">
                    <i class="bx bx-info-circle text-lg"></i> Panduan Ikon
                </div>
                <p class="text-xs text-purple-700 mb-2">Gunakan class dari <strong>Boxicons</strong>:</p>
                <div class="grid grid-cols-3 gap-2">
                    @foreach(['bx bx-user', 'bx bx-bus', 'bx bx-coin', 'bx bx-cog', 'bx bx-map', 'bx bx-tv'] as $ic)
                        <div class="flex flex-col items-center gap-1 cursor-pointer p-2 rounded-lg hover:bg-purple-100"
                            ...
                            <span class="text-[9px] text-gray-400 text-center leading-tight">{{ $ic }}</span>
                        </div>
                    @endforeach
                </div>
            </div>
